...
public class Responder extends DNSTask {
    ...
    public void start(Timer timer) {
        ...
        for (DNSQuestion question : _in.getQuestions()) {
            AndroidLogger.d("JmDNS." + getClass().getSimpleName(), this.getName() + "start() question=" + question);
        ...
        AndroidLogger.d("JmDNS." + getClass().getSimpleName(), this.getName() + "start() Responder chosen delay=" + delay);
    ...
    public void run() {
        ...
        if (this.getDns().isAnnounced()) {
            try {
                // Answer questions
                for (DNSQuestion question : _in.getQuestions()) {
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), this.getName() + "run() JmDNS responding to: " + question);
                ...
                for (DNSRecord knownAnswer : _in.getAnswers()) {
                    if (knownAnswer.isStale(now)) {
                        ...
                        AndroidLogger.d("JmDNS." + getClass().getSimpleName(), this.getName() + "JmDNS Responder Known Answer Removed");
                    }
                }

                // respond if we have answers
                if (!answers.isEmpty()) {
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), this.getName() + "run() JmDNS responding");
                ...
                AndroidLogger.w("JmDNS." + getClass().getSimpleName(), this.getName() + "run() exception ", e);
                this.getDns().close();
            }
        }
    }
