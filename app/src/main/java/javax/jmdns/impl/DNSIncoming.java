...
public final class DNSIncoming extends DNSMessage {
    ...
    public DNSIncoming(DatagramPacket packet) throws IOException {
        ...
        try {
            ...
            AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "DNSIncoming() dump " + print(true) + "\n exception ", e);
    ...
    private DNSQuestion readQuestion() {
        ...
        if (type == DNSRecordType.TYPE_IGNORE) {
            AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "Could not find record type: " + this.print(true));
        }
    ...
    private DNSRecord readAnswer(InetAddress source) {
        ...
        DNSRecordType type = DNSRecordType.typeForIndex(_messageInputStream.readUnsignedShort());
        if (type == DNSRecordType.TYPE_IGNORE) {
            AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "Could not find record type. domain: " + domain + "\n" + this.print(true));
        }
        int recordClassIndex = _messageInputStream.readUnsignedShort();
        ...
        if ((recordClass == DNSRecordClass.CLASS_UNKNOWN) && (type != DNSRecordType.TYPE_OPT)) {
            AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "Could not find record class. domain: " + domain + " type: " + type + "\n" + this.print(true));
        }
        boolean unique = recordClass.isUnique(recordClassIndex);
        ...
        switch (type) {
            ...
            case TYPE_PTR:
                ...
                if (service.length() > 0) {
                    ...
                    AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "PTR record of class: " + recordClass + ", there was a problem reading the service name of the answer for domain:" + domain);
                }
                break;
            case TYPE_TXT:
                ...
                if (version == 0) {
                    ...
                    while (_messageInputStream.available() > 0) {
                        ...
                        if (_messageInputStream.available() >= 2) {
                            ...
                            AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "There was a problem reading the OPT record. Ignoring.");
                            break;
                        }
                        int optionLength = 0;
                        if (_messageInputStream.available() >= 2) {
                        ...
                        switch (optionCode) {
                            case Owner:
                                ...
                                try {
                                    ...
                                    AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "Malformed OPT answer. Option code: Owner data: " + this._hexString(optiondata));
                                }
                                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "Unhandled Owner OPT version: " + ownerVersion + " sequence: " + ownerSequence + " MAC address: " + this._hexString(ownerPrimaryMacAddress)
                            ...
                            case NSID:
                            case UL:
                                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "There was an OPT answer. Option code: " + optionCode + " data: " + this._hexString(optiondata));
                                break;
                            case Unknown:
                                AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "There was an OPT answer. Not currently handled. Option code: " + optionCodeInt + " data: " + this._hexString(optiondata));
                                break;
                            default:
                    ...
                    AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "There was an OPT answer. Wrong version number: " + version + " result code: " + extendedResultCode);
                }
                break;
            default:
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "DNSIncoming() unknown type:" + type);
                ...
                switch (DNSLabel.labelForByte(len)) {
                    ...
                    case Compressed:
                        ...
                        if (compressedLabel == null) {
                            AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "bad domain name: possible circular name detected. Bad offset: 0x" + Integer.toHexString(index) + " at 0x" + Integer.toHexString(pos - 2));
                            compressedLabel = "";
                        }
                    ...
                    case Extended:
                        ...
                        AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "Extended label are not currently supported.");
                        break;
                    case Unknown:
                    default:
                        AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "unsupported dns label type: '" + Integer.toHexString(len & 0xC0) + "'");
                }
            }
