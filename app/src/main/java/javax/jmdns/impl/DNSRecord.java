...
public abstract class DNSRecord extends DNSEntry {
    ...
    boolean suppressedBy(DNSIncoming msg) {
        try {
            ...
            AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "suppressedBy() message " + msg + " exception ", e);
            // msg.print(true);
            return false;
        }
    }
    ...
    public static class IPv6Address extends Address {
        ...
        /*https://s.youtube.com/api/stats/watchtime?ns=yt&el=leanback&cpn=pyK1C5n0tQuxgdIU&docid=hXFv13gHtUs&ver=2&cmt=11.238&plid=AAUEZ14iL2iH0nEM&ei=OrgsVJKJO4GLyQPzwIDYAg&fmt=243&fs=0&rt=28.611&c=TVHTML5&cver=4&euri=https%3A%2F%2Fwww.youtube.com%2Ftv%3Fcastv%3D2.0%23%2Fwatch%3Fv%3Deps0J53sb_w%26mode%3Dtransport&lact=298&cplatform=TV&cbrand=Google&cmodel=Eureka&cbr=Chrome&cbrver=34.0.1847.44&cos=Android&cosver=4.4.2&cl=76791704&state=paused&final=1&hl=en_GB&cr=SE&len=137&list=RQcast&ctrl=mdx-cast&ytr=android-phone-5.10.3.5&idpj=-7&ldpj=-14&st=11.238&et=11.238
         * (non-Javadoc)
         * @see javax.jmdns.impl.DNSRecord#getServiceInfo(boolean)
         */
        @Override
    ...
    public static abstract class Address extends DNSRecord {
        ...
        protected Address(String name, DNSRecordType type, DNSRecordClass recordClass, boolean unique, int ttl, byte[] rawAddress) {
            ...
            try {
                ...
                AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "Address() exception ", exception);
            }
        }
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            if (dns.getLocalHost().conflictWithRecord(this)) {
                ...
                if (comparison == 0) {
                    ...
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleQuery() Ignoring an identical address query");
                    return false;
                }

                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleQuery() Conflicting query detected.");
                // Tie breaker test
        ...
        boolean handleResponse(JmDNSImpl dns) {
            if (dns.getLocalHost().conflictWithRecord(this)) {
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleResponse() Denial detected");
    ...
    public static class Service extends DNSRecord {
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            ...
            if (info != null && (info.isAnnouncing() || info.isAnnounced()) && (_port != info.getPort() || !_server.equalsIgnoreCase(dns.getLocalHost().getName()))) {
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleQuery() Conflicting probe detected from: " + getRecordSource());
                ...
                try {
                    if (dns.getInterface().equals(getRecordSource())) {
                        AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "Got conflicting probe from ourselves\n" + "incoming: " + this.toString() + "\n" + "local   : " + localService.toString());
                    }
                } catch (IOException e) {
                    AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "IOException", e);
                }
                ...
                if (comparison == 0) {
                    ...
                    // own records come in on different interfaces than the ones they were sent on.
                    // see section "10. Conflict Resolution" of mdns draft spec.
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleQuery() Ignoring a identical service query");
                    return false;
                }
                ...
                if (info.isProbing() && comparison > 0) {
                    ...
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleQuery() Lost tie break: new unique name chosen:" + info.getName());
        ...
        boolean handleResponse(JmDNSImpl dns) {
            ...
            if (info != null && (_port != info.getPort() || !_server.equalsIgnoreCase(dns.getLocalHost().getName()))) {
                ...
                if (info.isProbing()) {
                    ...
                    dns.getServices().remove(oldName);
                    dns.getServices().put(info.getQualifiedName().toLowerCase(), info);
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "handleResponse() New unique name chose:" + info.getName());
