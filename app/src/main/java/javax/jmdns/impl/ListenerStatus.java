...
public class ListenerStatus<T extends EventListener> {
    ...
    /**
     * @param listener listener being tracked.
     * @param synch    true if that listener can be called asynchronously
     */
    ...
    public static class ServiceListenerStatus extends ListenerStatus<ServiceListener> {
        ...
        void serviceAdded(ServiceEvent event) {
            ...
            if (null == _addedServices.putIfAbsent(qualifiedName, event.getInfo().clone())) {
                ...
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "Service Added called for a service already added: " + event);
            }
        }
        ...
        void serviceRemoved(ServiceEvent event) {
            ...
            if (_addedServices.remove(qualifiedName, _addedServices.get(qualifiedName))) {
                ...
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "Service Removed called for a service already removed: " + event);
            }
        }
        ...
        synchronized void serviceResolved(ServiceEvent event) {
            ...
            if ((info != null) && (info.hasData())) {
                ...
                if (!_sameInfo(info, previousServiceInfo)) {
                    ...
                    AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "Service Resolved called for a service already resolved: " + event);
                }
            } else {
                AndroidLogger.w("JmDNS." + getClass().getSimpleName(), "Service Resolved called for an unresolved event: " + event);
    ...
    public static class ServiceTypeListenerStatus extends ListenerStatus<ServiceTypeListener> {
        ...
        void serviceTypeAdded(ServiceEvent event) {
            if (null == _addedTypes.putIfAbsent(event.getType(), event.getType())) {
                ...
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "Service Type Added called for a service type already added: " + event);
            }
        }
        ...
        void subTypeForServiceTypeAdded(ServiceEvent event) {
            if (null == _addedTypes.putIfAbsent(event.getType(), event.getType())) {
                ...
                AndroidLogger.d("JmDNS." + getClass().getSimpleName(), "Service Sub Type Added called for a service sub type already added: " + event);
            }
        }
