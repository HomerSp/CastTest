    ...
    private HostInfo(final InetAddress address, final String name, final JmDNSImpl dns) {
        ...
        if (address != null) {
            try {
                ...
                AndroidLogger.e("JmDNS." + getClass().getSimpleName(), "LocalHostInfo() exception ", exception);
            }
        }
    }
        ...
        try {
            if (addr == null) {
                ...
                if (ip != null) {
                    ...
                    AndroidLogger.d("JmDNS." + "HostInfo", "newHostInfo getting address from interface");
                    for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements(); ) {
                        NetworkInterface intf = en.nextElement();
                        for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements(); ) {
                            InetAddress inetAddress = enumIpAddr.nextElement();
                            if (!inetAddress.isLoopbackAddress() && inetAddress instanceof Inet4Address) {
                                addr = InetAddress.getByName(inetAddress.getHostAddress());
                            }
                        }
                    }

                    if (addr == null) {
                        AndroidLogger.d("JmDNS." + "HostInfo", "Getting address from localhost");
                ...
                AndroidLogger.d("JmDNS." + "HostInfo", "newHostInfo: " + aName);

                if (addr.isLoopbackAddress()) {
                    AndroidLogger.w("JmDNS." + "HostInfo", "Could not find any address beside the loopback.");
                }
            } else {
                aName = addr.getHostName();
            }
            ...
            AndroidLogger.w("JmDNS." + "HostInfo", "Could not intialize the host network interface on " + address + "because of an error: " + e.getMessage(), e);
